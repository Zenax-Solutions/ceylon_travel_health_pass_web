            ...
            <form action="{{ route('destination.login.validate') }}" method="POST">
                ...
                <div class="w-full bg-white divide-y divide-gray-200 rounded-lg shadow">
                    <div class="px-5 py-7">
                        ...
                        <input type="text" required name="branch_code"
                            class="w-full px-3 py-2 mt-1 text-sm border rounded-lg"
                            placeholder="Enter Your Destination Branch Code" />
                        @error('branch_code')
                            <span class="mt-2 text-center text-red-400 invalid-feedback">{{ $message }}</span>
                        @enderror

                        <button type="submit"
                            class="transition mt-4 duration-200 bg-green-500 hover:bg-green-600 focus:bg-green-700 focus:shadow-sm focus:ring-4 focus:ring-green-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block">
