                    ...
                    <div class="h-full flex flex-col py-6 bg-white shadow-xl">
                        ...
                        <div class="p-4">
                            <div class="mb-4 flex space-x-4 text-sm p-2 bg-white rounded-lg shadow-md font-bold">
                                <button x-on:click="openTab = 1"
                                    :class="{ 'bg-green-600 font-bold text-white': openTab === 1 }"
                                    class="flex-1 py-2 px-4 rounded-md focus:outline-none font-bold focus:shadow-outline-blue transition-all duration-300">Destinations</button>
                                <button x-on:click="openTab = 2"
                                    :class="{ 'bg-green-600 font-bold text-white': openTab === 2 }"
                                    class="flex-1 py-2 px-4 rounded-md focus:outline-none font-bold focus:shadow-outline-blue transition-all duration-300">Services</button>
                                <button x-on:click="openTab = 3"
                                    :class="{ 'bg-green-600 font-bold text-white': openTab === 3 }"
                                    class="flex-1 py-2 px-4 rounded-md focus:outline-none font-bold focus:shadow-outline-blue transition-all duration-300">Shops</button>
                            </div>
                        </div>
                        <div class="mt-4 px-4 h-full overflow-auto">
                            <div x-show="openTab === 1" class="transition-all duration-300">
                                <!-- Search Input -->
                                <div class="mt-4 px-4">
                                    <input type="text" placeholder="Search Destination here" x-model="searchdestination"
                                        class="w-full p-2 border font-normal border-green-500 rounded-md focus:border-green-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-300 transition-colors duration-300">
                                </div>
                                <div class="mt-4 px-4">
                                ...
                                <div class="mt-4 px-4 h-full overflow-auto">
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        @forelse($destinations as $destination)
                                            <template
                                                x-if="searchdestination === '' || {{ json_encode($destination->destination) }}.toLowerCase().includes(searchdestination.toLowerCase())">
                                                <div
                                                    ...
                                                    <img style="object-fit: cover"
                                                        class="transform border border-gray-200 rounded-md w-full h-20 object-cover"
                                                        src="{{ Storage::url($destination->image) }}" />
                                                    <br>
                                                    @if(!empty($destination->discount_info))
                                                        <span
                                                            class="inline-flex flex-shrink-0 items-center rounded-full bg-red-600 px-1.5 py-0.5 text-xs font-medium text-white ring-1 ring-inset ring-red-600/20">{{$destination->discount_info}}</span>
                            ...
                            <div x-show="openTab === 2" class="transition-all duration-300">
                                <!-- Search Input -->
                                <div class="mt-4 px-4">
                                    <input type="text" placeholder="Search Services here" x-model="searchservices"
                                        class="w-full p-2 border font-normal border-green-500 rounded-md focus:border-green-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-300 transition-colors duration-300">
                                </div>
                                <div class="mt-4 px-4">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        @if(!empty($package->discount_service_list))
                                            @forelse($package->discount_service_list as $serviceId)
                                                ...
                                                @endphp
                                                <template
                                                    x-if="searchservices === '' || {{ json_encode($service?->service_name) }}.toLowerCase().includes(searchservices.toLowerCase())">
                                                    <div
                                                        class="bg-gray-50 hover:bg-gray-100 p-4 cursor-pointer rounded-md border border-gray-300 transition-colors duration-300">
                                                        <h3 class="text-sm font-semibold text-black mb-2">
                            ...
                            <div x-show="openTab === 3" class="transition-all duration-300">
                                <!-- Search Input -->
                                <div class="mt-4 px-4">
                                    <input type="text" placeholder="Search Shops here" x-model="searchshops"
                                        class="w-full p-2 border font-normal border-green-500 rounded-md focus:border-green-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-300 transition-colors duration-300">
                                </div>
                                <div class="mt-4 px-4">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        @if(!empty($package->discount_shop_list))
                                            @forelse($package->discount_shop_list as $shopId)
                                                ...
                                                @endphp
                                                <template
                                                    x-if="searchshops === '' || {{ json_encode($shop?->shope_name) }}.toLowerCase().includes(searchshops.toLowerCase())">
                                                    <div
                                                        class="bg-gray-50 hover:bg-gray-100 p-4 cursor-pointer rounded-md border border-gray-300 transition-colors duration-300">
                                                        <h3 class="text-sm font-semibold text-black mb-2">
                                                            {{ $shop?->shope_name }}</h3>
