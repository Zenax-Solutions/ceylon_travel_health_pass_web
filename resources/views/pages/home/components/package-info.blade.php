    ...
    <div x-data="{ open: false, search: '' }">
            ...
            <section class="absolute inset-y-0 right-0 pl-10 max-w-full flex">
                <div x-show="open" x-transition:enter="transition-transform ease-out duration-300" x-transition:enter-start="transform translate-x-full" x-transition:enter-end="transform translate-x-0" x-transition:leave="transition-transform ease-in duration-300" x-transition:leave-start="transform translate-x-0" x-transition:leave-end="transform translate-x-full" class="w-screen max-w-md">
                    <div class="h-full flex flex-col py-6 bg-white shadow-xl">
                        ...
                        <!-- Search Input -->
                        <div class="mt-4 px-4">
                            <input type="text" placeholder="Search Destination here" x-model="search" class="w-full p-2 border font-normal border-green-500 rounded-md focus:border-green-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-300 transition-colors duration-300">
                        </div>
                        <div class="mt-4 px-4">
                        ...
                        <div class="mt-4 px-4 h-full overflow-auto">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <template x-if="search === '' || {{ json_encode($destination->destination) }}.toLowerCase().includes(search.toLowerCase())">
        ...
        <div>
            <div class="inline-block align-bottom">
                <a href="#" x-on:click="open = true" class="middle none center mr-3 rounded-lg bg-green-500 py-3 px-6 font-sans text-xs font-bold text-white shadow-md shadow-green-500/20 transition-all hover:shadow-lg hover:shadow-green-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none" data-ripple-light="true">
                    More Details
                </a>
            </div>
        </div>
